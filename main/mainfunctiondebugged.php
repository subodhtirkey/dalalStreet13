...
//				return "kick";
				...
				$thesellersharesofthiskindcurrently=mysql_query("SELECT `num` FROM 	`stocks_details` WHERE `userid`='{$theselluserid}' AND `stockid`='{$thebuyuserid}'");//buy user id and sell user id is thesame
				...
				$nameofthestock=mysql_query("SELECT `stockname` FROM `stocks` WHERE `stockid`='{$thebuystockidis}'");
				if(mysql_num_rows($nameofthestock))
				{
					//the stockname could not be retrived
				}
				$nameofthestock_fetch=mysql_fetch_array($nameofthestock);
				$nameofthestock_actual=$nameofthestock_fetch[0];
